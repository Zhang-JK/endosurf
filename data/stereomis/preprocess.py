...
def create_stereomis_info(dset_dir, info_dir, test_every=8, object_scale_in_sphere=0.6, show=False):
    ...
    # fix camera coordinate for stereomis
    # poses[:, :, 0] = -poses[:, :, 0]
    # poses[:, :, 2] = -poses[:, :, 2]
    poses[:, :, 0] = -poses[:, :, 0]
    poses[:, :, 1] = -poses[:, :, 1]
    ...
    #     poses[i_frame, :, :4] = np.linalg.inv(np.vstack([poses[i_frame, :, :4], np.array([[0,0,0,1]])]))[:3, :]
    ...
    inf_depth = np.percentile(depths[depths!=0], 97)
    ...
    if show:
        ...
        o3d.visualization.draw_geometries([pcds, coord, sphere])
...
def main():
    ...
    parser.add_argument("--dset_dir", default="data/stereomis/dataset/pulling_soft_tissues",
                        type=str, help="dataset path")
    parser.add_argument("--info_dir", default="data/data_info/stereomis/",
    ...
    create_stereomis_info(args.dset_dir, args.info_dir, args.test_every, args.object_scale_in_sphere, args.show)
